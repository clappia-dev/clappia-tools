...
class WorkflowLastUpdatedBy(BaseModel):
    ...
    email_address: str = Field(
        description="Email address of the user who last updated the workflow"
    )


class WorkflowResponse(BaseResponse):
    app_id: str = Field(None, description="App ID")
    trigger_definition: Optional[WorkflowTriggerDefinition] = Field(
        None, description="Workflow trigger definition"
    )
    steps: Optional[List[WorkflowStep]] = Field(
        None, description="List of workflow steps"
    )
    last_updated_by: Optional[WorkflowLastUpdatedBy] = Field(
        None, description="User who last updated the workflow"
    )
